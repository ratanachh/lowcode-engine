...
- **Use the official API**: Check [workspace.isActive](/lowcode-engine/docs/api/workspace#isactive) to see whether IDE mode is active. This official lowcode engine API confirms whether you are in the integrated development environment.
...
- **Set resource list API**: In IDE mode, use [workspace.setResourceList](/lowcode-engine/docs/api/workspace#setresourcelist) to set or update the resource list in the IDE so resources opened in editor windows are current and accessible.

#### How do I open an editor view window?

- **Recommended approach**: Use `openEditorWindow(resource: Resource, sleep?: boolean): Promise<void>;` to open an editor window. **resource** is the specific resource to open; get it from [workspace.resourceList](/lowcode-engine/docs/api/workspace#resourcelist).
